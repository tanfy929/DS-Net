...
def RocCall(test_dir):
    # test_dir = 'TestResult/TheNew2/'
    ifLarge = 0

    for i in range(26):
        data = scio.loadmat(test_dir + ('_%s' % (i + 1)))

        print(test_dir + ('_%s' % (i + 1)))
        outZ = data['outZ']
        ...
        if ifLarge == 1:
            ...
            if i == 0:
                outZall = np.reshape(outZ, [712*1072, 5])
                truZall = np.reshape(orlZ, [712*1072, 5])

            else:
                outZall = np.vstack((outZall, np.reshape(outZ, [712*1072, 5])))
                truZall = np.vstack((truZall, np.reshape(orlZ, [712*1072, 5])))
